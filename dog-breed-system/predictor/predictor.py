...
class Predictor:
    ...
    def process_output(self, out):
        ...
        return self.labels[classId], out[classId]
    ...
    def classify(self, img):
        return self.process_output(self.predict(img))

    def classify_topn(self, img, n):
        predictions = self.predict(img)
        topn_classes = predictions[0].argsort()[-n:][::-1]
        return [(self.labels[id_], predictions[0][id_]) for id_ in topn_classes]
